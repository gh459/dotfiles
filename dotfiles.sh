...
INSTALL_YAY_CHROOT=${install_yay_chroot_esc} # yayインストール選択変数
...
if [ "${INSTALL_YAY_CHROOT}" = "yes" ]; then
    ...
    sudo -u "${USERNAME_CHROOT}" bash -ec '
        ...
        build_dir="$HOME/yay_build_temp_$(date +%s)"
        mkdir -p "${build_dir}"
    ...
    # ProtonUp-Qt と Google Chrome のインストール (yay経由)
