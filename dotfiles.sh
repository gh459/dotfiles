...
CONFIG_FILE_ENHANCED="arch_install_config_enhanced.conf" # <<< 拡張設定ファイル
...
# <<< 拡張設定ファイルから読み込まれる変数 (デフォルト値は設定しない) >>>
PREFERRED_SHELL_CONFIG=""
INSTALL_OHMYZSH_CONFIG=""
INSTALL_STEAM_CHOICE_CONFIG=""
INSTALL_PROTONUPQT_CHOICE_CONFIG=""
INSTALL_CHROME_CHOICE_CONFIG=""
ADDITIONAL_NETWORK_PACKAGES_CONFIG=""
ADDITIONAL_BLUETOOTH_PACKAGES_CONFIG=""
ADDITIONAL_PRINTER_PACKAGES_CONFIG=""
ENABLE_FIREWALLD_SERVICE_CONFIG=""
ENABLE_BLUETOOTH_SERVICE_CONFIG=""
ENABLE_CUPS_SERVICE_CONFIG=""
...
load_config() {
    ...
    # <<< 拡張設定ファイルの読み込みを追加 >>>
    if [ -f "$CONFIG_FILE_ENHANCED" ]; then
        source "$CONFIG_FILE_ENHANCED"
        log_info "Loaded enhanced configuration from $CONFIG_FILE_ENHANCED."
    else
        log_warn "Enhanced configuration file $CONFIG_FILE_ENHANCED not found. Using default values and prompts for enhanced settings."
    fi
}
...
prompt_initial_settings() {
    ...
    # <<< 拡張設定のプロンプト (設定ファイルの値があればそれをデフォルトにする) >>>
    local default_steam_choice="${INSTALL_STEAM_CHOICE_CONFIG:-no}"
    echo "Do you want to install Steam? (yes/no) [Default: $default_steam_choice]"
    read -r user_steam_choice
    INSTALL_STEAM_CHOICE="${user_steam_choice:-$default_steam_choice}"

    local default_protonupqt_choice="${INSTALL_PROTONUPQT_CHOICE_CONFIG:-no}"
    echo "Do you want to install ProtonUp-Qt (via yay)? (yes/no) [Default: $default_protonupqt_choice]"
    read -r user_protonupqt_choice
    INSTALL_PROTONUPQT_CHOICE="${user_protonupqt_choice:-$default_protonupqt_choice}"

    local default_chrome_choice="${INSTALL_CHROME_CHOICE_CONFIG:-no}"
    echo "Do you want to install Google Chrome (via yay)? (yes/no) [Default: $default_chrome_choice]"
    read -r user_chrome_choice
    INSTALL_CHROME_CHOICE="${user_chrome_choice:-$default_chrome_choice}"
...
configure_environment_packages() {
    ...
    local final_pkg_candidate_list=() # 結合前のリスト... # 1. Desktop Environment Packages
    ...
    # <<< 拡張設定ファイルからのパッケージリスト追加 >>>
    # 各設定変数が空白でない場合に、スペース区切りの文字列を配列に変換して追加
    add_packages_from_config() {
        local config_var_name="$1" # 設定変数名
        local pkg_list_array_name="$2" # 結果を格納する配列名 (local で定義すること)
        local config_value="${!config_var_name}" # 変数名から値を取得

        if [ -n "$config_value" ]; then
            IFS=' ' read -r -a pkg_array <<< "$config_value"
            if [ ${#pkg_array[@]} -gt 0 ]; then
                declare -n pkg_list_array="$pkg_list_array_name" # 名前参照で配列を扱う
                pkg_list_array+=("${pkg_array[@]}") # 既存の配列に追加
            fi
        fi
    }

    local additional_network_pkg_list=()
    add_packages_from_config "ADDITIONAL_NETWORK_PACKAGES_CONFIG" "additional_network_pkg_list"
    final_pkg_candidate_list+=("${additional_network_pkg_list[@]}")

    local additional_bluetooth_pkg_list=()
    add_packages_from_config "ADDITIONAL_BLUETOOTH_PACKAGES_CONFIG" "additional_bluetooth_pkg_list"
    final_pkg_candidate_list+=("${additional_bluetooth_pkg_list[@]}")

    local additional_printer_pkg_list=()
    add_packages_from_config "ADDITIONAL_PRINTER_PACKAGES_CONFIG" "additional_printer_pkg_list"
    final_pkg_candidate_list+=("${additional_printer_pkg_list[@]}")

    # Steamのインストール選択
    if [ "$INSTALL_STEAM_CHOICE" = "yes" ]; then
        final_pkg_candidate_list+=("steam")
    fi
...
confirm_installation() {
    ...
    # <<< 拡張設定の確認表示を追加 >>>
    log_warn "--- Enhanced Configuration ---"
    log_warn "Steam: ${INSTALL_STEAM_CHOICE:-Not Selected}"
    log_warn "ProtonUp-Qt: ${INSTALL_PROTONUPQT_CHOICE:-Not Selected} (requires yay)"
    log_warn "Chrome: ${INSTALL_CHROME_CHOICE:-Not Selected} (requires yay)"
    log_warn "Preferred Shell: ${PREFERRED_SHELL_CONFIG:-bash}"
...
generate_chroot_script() {
    ...
    # <<< 拡張設定を渡す >>>
    local preferred_shell_chroot_esc=$(printf '%q' "${PREFERRED_SHELL_CONFIG}")
    local install_ohmyzsh_chroot_esc=$(printf '%q' "${INSTALL_OHMYZSH_CONFIG}")
    local install_steam_chroot_esc=$(printf '%q' "${INSTALL_STEAM_CHOICE}")
    local install_protonupqt_chroot_esc=$(printf '%q' "${INSTALL_PROTONUPQT_CHOICE}")
    local install_chrome_chroot_esc=$(printf '%q' "${INSTALL_CHROME_CHOICE}")
    local enable_firewalld_chroot_esc=$(printf '%q' "${ENABLE_FIREWALLD_SERVICE_CONFIG}")
    local enable_bluetooth_chroot_esc=$(printf '%q' "${ENABLE_BLUETOOTH_SERVICE_CONFIG}")
    local enable_cups_chroot_esc=$(printf '%q' "${ENABLE_CUPS_SERVICE_CONFIG}")
